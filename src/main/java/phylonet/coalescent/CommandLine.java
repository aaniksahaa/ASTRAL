...
public class CommandLine {
	...
	private static SimpleJSAP getJSAP() throws JSAPException {
		return new SimpleJSAP("ASTRAL (version" + _version + ")",
				...
				new Parameter[] { new FlaggedOption("input file", FileStringParser.getParser().setMustExist(true), null,
						...
						new FlaggedOption("lambda", JSAP.DOUBLE_PARSER, "0.5", JSAP.NOT_REQUIRED, 'c', "lambda",
						...
						new FlaggedOption("samplingrounds", JSAP.INTEGER_PARSER, null, JSAP.NOT_REQUIRED,
						...
						new FlaggedOption("gene repetition", JSAP.INTEGER_PARSER, "1", JSAP.NOT_REQUIRED, 'w',
								"generepeat", "the number of trees sampled for each locus. "),
						...
						new FlaggedOption("extraLevel", JSAP.INTEGER_PARSER, "1", JSAP.NOT_REQUIRED, 'p', "extraLevel",
								"How much extra bipartitions should be added: 0, 1, or 2. " + "0: adds nothing extra. "
						...
						new FlaggedOption("extra trees", FileStringParser.getParser().setMustExist(true), null,
								JSAP.NOT_REQUIRED, 'e', "extra",
								"provide extra trees (with gene labels) used to enrich the set of clusters searched"),
						new FlaggedOption("extra species trees", FileStringParser.getParser().setMustExist(true), null,
								JSAP.NOT_REQUIRED, 'f', "extra-species",
	...
	static Options readOptions( boolean rooted, boolean extrarooted, double wh, JSAPResult config,
				...
				while ((s = br.readLine()) != null) {
					...
					for (String allele : alleles) {
						...
						if (taxonMap.containsKey(allele)) {
							...
							Logging.log("A gene name can map to one only species name; check: " + allele
									+ " which seems to appear at least twice: " + taxonMap.get(allele) + " & "
									+ species);
							System.exit(-1);
		...
		if (config.getStringArray("keep") != null && config.getStringArray("keep").length != 0) {
			...
			for (String koption : config.getStringArray("keep")) {
				if ("completed".equals(koption) || "bootstraps".equals(koption) || "bootstraps_norun".equals(koption)
						|| "searchspace_norun".equals(koption) || "searchspace".equals(koption)) {
		...
		if (config.getFile("bootstraps") != null || config.getBoolean("gene-only")) {
			...
			for (int i = 0; i < config.getInt("replicates"); i++) {
				...
				try {
					...
					exitWithErr("Error: You seem to have asked for " + config.getInt("replicates") + " but only " + i
				...
				if (keepOptions.contains("bootstraps_norun") || keepOptions.contains("bootstraps")) {
			...
			if (keepOptions.contains("bootstraps_norun") || keepOptions.contains("bootstraps")) {
				Logging.log("bootstrap files written to files " + outfile + ("." + 0 + ".bs") + " to " + outfile
		...
		Options options = new Options(rooted, extrarooted, config.getBoolean("exact"), 1,
				...
				config.getDouble("lambda"), outfileName, samplingrounds == null ? -1 : samplingrounds,
				polylimit == null ? -1 : polylimit, config.getDouble("trimming threshold"), freqPath, minleaves,
				config.getInt("gene repetition"), 
				config.contains("remove extra tree bipartitions"), config.getBoolean("internode-dist"));
	...
	private static void runScore( boolean rooted, List<Tree> mainTrees, BufferedWriter outbuffer,
		...
		for (String trs : toScore) {
			...
			readInputTrees(trees, Arrays.asList(new String[] { trs }), rooted, true, true, null, 1, false ? 
					outgroup : null);
			Tree tr = trees.get(0);
		...
		try {
			if (config.getFile("extra trees") != null) {
				readInputTrees(extraTrees, readTreeFileAsString(config.getFile("extra trees")), extrarooted, true,
						false, null, 1, null);
				Logging.log(extraTrees.size() + " extra trees read from " + config.getFile("extra trees"));
			}
	...
	private static void readInputTrees(List<Tree> trees, List<String> lines, boolean rooted, boolean checkCompleteness,
			boolean stLablel, Integer minleaves, int annotation, String outgroup)
		...
		if (skipped.size() > 0) {
			Logging.log(
					"Skipping the following tree(s) because they had less than " + minleaves + " leaves: \n" + skipped);
		}
	}
